  ...
  describe HarvestJob do
    ...
    describe :process_feed! do
      ...
      it 'defaults to harvesting all records' do
        ...
        expect(harvester).to receive(:harvest).with(from_time: nil, until_time: nil)
        job.process_feed!
      end

      it 'prefers an explicit start time, if set' do
        from_time = Time.now
        expect(harvester).to receive(:harvest).with(from_time: from_time, until_time: nil)
        job.process_feed!(from_time: from_time)
      end

      it 'accepts an explicit end time' do
        until_time = Time.now
        expect(harvester).to receive(:harvest).with(hash_including(until_time: until_time))
      ...
      it 'prefers the "oldest failed" time, if no explicit start time set' do
        ...
        expect(harvester).to receive(:harvest).with(hash_including(from_time: lh.oldest_failed_datestamp))
        job.process_feed!
      end
      ...
      it 'falls back to the "newest success" time, if no explicit start time or "oldest failed" set' do
        ...
        expect(harvester).to receive(:harvest).with(hash_including(from_time: lh.newest_success_datestamp))
        job.process_feed!
      end
    end
  end
